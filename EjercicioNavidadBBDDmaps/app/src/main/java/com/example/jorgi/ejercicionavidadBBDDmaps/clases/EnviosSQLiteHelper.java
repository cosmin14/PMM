...
public class EnviosSQLiteHelper extends SQLiteOpenHelper {
    ...
    private static final String PEDIDO_DESTINOID = "destinoId";
    private static final String PEDIDO_PESO = "peso";
    private static final String PEDIDO_TARIFAPESO = "tarifaPeso";
    private static final String PEDIDO_EXPRESS = "express";
    ...
    String sqlCreatePedidos = "CREATE TABLE IF NOT EXISTS `pedidos` (" +
            "  `"+KEY_CODIGO+"` INTEGER NOT NULL PRIMARY KEY," +
            "  `"+PEDIDO_USUARIODNI+"` TEXT NOT NULL," +
            "  `"+PEDIDO_DESTINOID +"` TEXT NOT NULL," +
            "  `"+PEDIDO_PESO+"` TEXT NOT NULL," +
            "  `"+PEDIDO_TARIFAPESO+"` TEXT NOT NULL," +
            "  `"+PEDIDO_EXPRESS+"` REAL NOT NULL," +
            "   FOREIGN KEY(`usuarioDNI`) REFERENCES usuarios("+ USUARIO_DNI +")," +
            "   FOREIGN KEY(`"+PEDIDO_DESTINOID+"`) REFERENCES destinos("+ KEY_CODIGO +")" +
            "  );";
    ...
    * */
     ...
     * @return
     */
    public Usuario getUsuarioByEmail(String usuario_email) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    * */
    ...
    public long crearPedido(Pedido pedido) {
        ...
        values.put(PEDIDO_DESTINOID, pedido.getZonaId());
        values.put(PEDIDO_PESO, pedido.getPeso());
        values.put(PEDIDO_TARIFAPESO, pedido.getTarifaPeso());
        values.put(PEDIDO_EXPRESS, pedido.getExpress());
    ...
    public List<Pedido> getAllPedidos() {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                td.setZonaId(c.getString(c.getColumnIndex(PEDIDO_DESTINOID)));
                td.setPeso(c.getString(c.getColumnIndex(PEDIDO_PESO)));
                td.setTarifaPeso(c.getString(c.getColumnIndex(PEDIDO_TARIFAPESO)));
                td.setExpress(c.getFloat(c.getColumnIndex(PEDIDO_EXPRESS)));
                // adding to Usuario list
                pedidos.add(td);
            } while (c.moveToNext());
        }
    ...
    * */
    ...
    public Destino getDestinoById(int id){
        ...
        String selectQuery = "SELECT * FROM " + TABLA_DESTINOS + " WHERE "+ DESTINO_CODIGO +" = "+id;
        ...
        if (c.moveToFirst()){
            do {
                ...
                td.setNombre((c.getString(c.getColumnIndex(DESTINO_NOMBRE))));
                td.setZona(c.getString(c.getColumnIndex(DESTINO_ZONA)));
                td.setPrecio(c.getInt(c.getColumnIndex(DESTINO_PRECIO)));
                td.setImagen(c.getColumnIndex(DESTINO_IMAGEN));
                c.close();
            }while (c.moveToNext());
        }
    ...
    public int selectCountDestinos(){
